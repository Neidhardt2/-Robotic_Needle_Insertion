...
kernel = np.zeros((number_of_kernels, 3), dtype=np.int_)
counter = 0
...
def getLine(fromPoint, throughPoint, maxPoint, minPoint, inc=1, asType=np.int_):
    gradient = np.asarray(throughPoint - fromPoint, dtype=float)
